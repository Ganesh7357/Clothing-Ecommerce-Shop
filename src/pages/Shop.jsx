...
export const shopLoader = async ({ request }) => {
  ...
  try {
    ...
    console.log('response=+++++++++++++1234567890+===>', response);
    const data = response?.products;
    return { productLoaderData: data || [] }; // Return products or an empty array if no products found
  } catch (err) {
    console.log('err====>', err);
    return { productLoaderData: [] }; // Return an empty array in case of an error
  }
